...
void loop()
{
	...
	while (1)
	{
		...
		else if (strcmp(choose, "address") == 0)
		{
...
//	>>>	accelerate 加速(V)  decelerate 減速(V)
void acc_dec()
{	
	unionType Slave1;
	unionType Slave2;
	...
	if (Max_Speed != 0)
	{
		/*for (int i = 0; i < 4; i++)
		{
			...
			{
				Wire.write(Slave1.a[i2]);
				MySerial.println(Slave1.a[i2]);
			}
			Wire.endTransmission();
		}	*/	
		Wire.beginTransmission(MySlave1);
		...
		while (facc_acc != 1)
		{
			Wire.requestFrom(MySlave1, 4);
			facc_acc = Wire.read();
			delay(30);
		}
		if (debug)
		{
			MySerial.println("acc_dec_success");
		}		
	}	
}	//	<<<	accelerate 加速(V)  decelerate 減速(V)
...
void EEPROM_Slave_Address()	//	>>>	重訂子版地址
{	
	for (int i = 0; i < 4; i++)
	{
		EEPROM_ADDRESS[i] = 0;
	...
	if (debug)
	{
		MySerial.println("*->EEPROM_Slave_Address ");
	}
	for (int i = 0; i < 4; i++)
	{
